...
class _wrapped_collection(_wrapped_object, pymongo.collection.Collection):
    ...
    find = lambda self, *args, **kwargs: _wrapped_cursor(self._reconnect(pymongo.collection.Collection.find, *args, **kwargs), slave_okay=self._slave_okay)
    ...
    def find_one(self, *args, **kwargs):
        ...
        try:
            ret = self._reconnect(pymongo.collection.Collection.find_one, *args, **kwargs)
...
class mongo(object):
    ...
    def connection(self):
        if self._connection is None:
            if self._rsname is not None:
                self._connection = pymongo.ReplicaSetConnection(','.join(self._hosts), max_pool_size=1, replicaSet=self._rsname, read_preference=pymongo.ReadPreference.SECONDARY)
            else:
